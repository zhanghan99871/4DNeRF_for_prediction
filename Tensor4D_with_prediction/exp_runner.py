...
class Runner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False, predict_num=5):
        ...
        self.predict_freq = self.conf.get_int('train.predict_freq')
    ...
    def train(self, render_only):
        ...
        image_perm = self.get_train_image_perm()
        image_test_perm = self.get_test_image_perm()
        self.dataset.update(self.predict_num)

        # print(image_perm)
        # print(image_test_perm)
        # return
        ...
        if render_only: 
            ...
            for i in range(self.predict_num): 
                self.validate_image(idx=i+len(image_perm), target_dir="{}/result".format(self.conf['dataset.data_dir']), resolution_level=1, is_last=True)
            return
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            if self.iter_step % self.predict_freq == 0: 
                for i in image_test_perm.numpy(): 
                    self.validate_image(idx=i, target_dir="{}/predict".format(self.conf['dataset.data_dir']), resolution_level=-1)
                for i in range(self.predict_num): 
                    self.validate_image(idx=i+len(image_perm)+len(image_test_perm), target_dir="{}/future".format(self.conf['dataset.data_dir']), resolution_level=-1, is_last=True)
        ...
        for i in image_test_perm.numpy(): 
            self.validate_image(idx=i, target_dir="{}/result".format(self.conf['dataset.data_dir']), resolution_level=1, is_last=True)
        for i in range(self.predict_num): 
            self.validate_image(idx=i+len(image_perm)+len(image_test_perm), target_dir="{}/future".format(self.conf['dataset.data_dir']), resolution_level=1, is_last=True)

    def get_train_image_perm(self):
        return torch.arange(0, int(self.dataset.n_images * self.train_test_split)).long()

    def get_test_image_perm(self):
        return torch.arange(int(self.dataset.n_images * self.train_test_split), self.dataset.n_images).long()
    ...
    def validate_image(self, idx=-1, resolution_level=-1, target_dir=None, is_last=False):
        if idx < 0:
            ...
            for i in range(img_fine.shape[-1]):
                ...
                if len(out_normal_fine) > 0:
                    cv.imwrite(os.path.join(self.base_exp_dir,
                                            ...
                                            '{:0>8d}_{}_{}.png'.format(self.iter_step, i, idx)),
                            normal_img[..., i])
                psnr = mse2psnr(img2mse(torch.Tensor(np.concatenate(out_rgb_fine, axis=0).reshape([H, W, 3, -1]))[..., i], 
                    torch.Tensor(self.dataset.image_at(idx, resolution_level=resolution_level)) / 256.0))
                print("PSNR: {}".format(psnr))

        else: 
            if is_last: 
                os.makedirs(os.path.join(target_dir, 'validations_fine_{}'.format(self.train_test_split)), exist_ok=True)
                cv.imwrite(os.path.join(target_dir, 'validations_fine_{}'.format(self.train_test_split), '{:0>8d}_{}.png'.format(self.iter_step, idx)),
                                            img_fine[..., 0])
            else:
                os.makedirs(os.path.join(target_dir, 'validations_fine_{}'.format(self.train_test_split)), exist_ok=True)
                psnr = mse2psnr(img2mse(torch.Tensor(np.concatenate(out_rgb_fine, axis=0).reshape([H, W, 3, -1]))[..., 0], 
                    torch.Tensor(self.dataset.image_at(idx, resolution_level=resolution_level)) / 256.0))
                f = os.path.join(target_dir, 'validations_fine_{}'.format(self.train_test_split), 'predict_psnr.txt')
                with open(f, 'a') as file:
                    file.write('Epoch: {}, index: {}, PSNR: {}\n'.format(self.iter_step, idx, psnr))
                cv.imwrite(os.path.join(target_dir, 'validations_fine_{}'.format(self.train_test_split), '{:0>8d}_{}.png'.format(self.iter_step, idx)),
                                            np.concatenate([img_fine[..., 0],
                                            self.dataset.image_at(idx, resolution_level=resolution_level)]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predict_num', type=int, default=5)
    ...
    runner = Runner(args.conf, args.mode, args.case, args.is_continue, args.predict_num)

    if args.mode == 'train':
        runner.train(render_only=args.render_only)
