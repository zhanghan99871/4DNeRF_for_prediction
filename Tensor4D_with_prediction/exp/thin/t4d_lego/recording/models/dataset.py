...
class Dataset:
    def __init__(self, conf):
        ...
        self.images_np = np.stack([cv.resize(cv.imread(im_name), (800, 800), interpolation=cv.INTER_LINEAR) for im_name in self.images_lis]) / 256.0
        self.masks_lis = sorted(glob(os.path.join(self.data_dir, 'mask/*.png')))
        self.masks_np = np.stack([cv.resize(cv.imread(im_name), (800, 800), interpolation=cv.INTER_LINEAR) for im_name in self.masks_lis]) / 256.0
    ...
    def update(self, predict_num):
        self.predict_num = predict_num

        # world_mat is a projection matrix from world to image
        self.world_mats_np = self.world_mats_np + [self.camera_dict['world_mat_%d' % (self.n_images - 1)].astype(np.float32) for idx in range(self.predict_num)]
        self.fid_list = self.fid_list + [torch.LongTensor(np.array([self.camera_dict['fid_%d' % (self.n_images - 1)]])) for idx in range(self.predict_num)]

        # scale_mat: used for coordinate normalization, we assume the scene to render is inside a unit sphere at origin.
        self.scale_mats_np = self.scale_mats_np + [self.camera_dict['scale_mat_%d' % (self.n_images - 1)].astype(np.float32) for idx in range(self.predict_num)]
