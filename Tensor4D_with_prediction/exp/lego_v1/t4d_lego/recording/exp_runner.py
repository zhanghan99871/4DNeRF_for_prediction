    ...
    def train(self, render_only):
        ...
        if render_only: 
            ...
            for i in range(self.predict_num): 
                self.validate_image(idx=i+len(image_perm), target_dir="../data/lego_v1/result", resolution_level=1, is_last=True)
            return
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            if self.iter_step % self.predict_freq == 0: 
                for i in image_test_perm.numpy(): 
                    self.validate_image(idx=i, target_dir="../data/lego_v1/predict", resolution_level=-1)
                for i in range(self.predict_num): 
                    self.validate_image(idx=i+len(image_perm)+len(image_test_perm), target_dir="../data/lego_v1/future", resolution_level=-1, is_last=True)
        ...
        for i in image_test_perm.numpy(): 
            self.validate_image(idx=i, target_dir="../data/lego_v1/result", resolution_level=1, is_last=True)
            for i in range(self.predict_num): 
                self.validate_image(idx=i+len(image_perm)+len(image_test_perm), target_dir="../data/lego_v1/future", resolution_level=1, is_last=True)
