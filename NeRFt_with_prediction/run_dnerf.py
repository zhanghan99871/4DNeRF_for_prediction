...
def batchify(fn, chunk):
    ...
    def ret(inputs_pos, inputs_time):
        ...
        for i in range(0, num_batches, chunk):
            out = fn(inputs_pos[i:i+chunk], [inputs_time[0][i:i+chunk], inputs_time[1][i:i+chunk]])
            out_list += [out]
...
def run_network(inputs, viewdirs, frame_time, fn, embed_fn, embeddirs_fn, embedtime_fn, netchunk=1024*64,
                embd_time_discr=True, half_precision=False):
    ...
    if half_precision:
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            outputs_flat = batchify(fn, netchunk)(embedded, embedded_times)
...
def create_nerf(args):
    ...
    network_query_fn = lambda inputs, viewdirs, ts, network_fn : run_network(inputs, viewdirs, ts, network_fn,
                                                                ...
                                                                half_precision=args.do_half_precision)
...
def config_parser():
    ...
    parser.add_argument("--chunk", type=int, default=2048*32, 
                        help='number of rays processed in parallel, decrease if running out of memory')
    parser.add_argument("--netchunk", type=int, default=2048*64, 
    ...
    parser.add_argument("--i_predict", type=int, default=10000,
...
def train():
    ...
    f = os.path.join(args.basedir, args.expname, 'predict_psnr.txt')
    if os.path.exists(f): 
        os.remove(f)
    ...
    if args.dataset_type == 'blender':
        images, poses, times, render_poses, render_times, hwf, i_split = load_blender_data(args.datadir, args.half_res, args.testskip, predict_num)
        ...
        i_target = []
        for each in i_train: 
            if times[each] > time_threshold: 
                train_list.append(each) 
        for each in train_list: 
            i_train.remove(each)
            i_target.append(each)


        i_train, i_target = np.array(i_train), np.array(i_target)
        # print(i_train)
        # print(i_target)
        # print(times)
        # return
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            i_train, i_target = i_train.tolist(), i_target.tolist()
            images = torch.Tensor(images).to(device)
            poses = torch.Tensor(poses).to(device)
            times = torch.Tensor(times).to(device)
            for img_i in i_train + i_target:
                print(img_i)
                target = images[img_i]
                ...
                f = os.path.join(basedir, expname, 'predict_psnr.txt')
                with open(f, 'a') as file:
                    file.write('Epoch: {}, index: {}, PSNR: {}\n'.format(200000, img_i, psnr))

                predict_image = Image.fromarray(to8b(rgb.cpu().numpy()))
                
                img_path = os.path.join(args.outputdir, 'r_{:02d}.png'.format(img_i))
                predict_image.save(img_path)
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_predict==0:
            torch.cuda.empty_cache()
            # Log a rendered validation view to Tensorboard
            for img_i in i_target:
                target = images[img_i]
                pose = poses[img_i, :3,:4]
                frame_time = times[img_i]
                with torch.no_grad():
            ...
            for idx in range(predict_num):
                pose = poses[i_target[-1], :3,:4]
                frame_time = times[i_target[-1] + idx]
                with torch.no_grad():
                    rgb, disp, acc, extras = render(H, W, focal, chunk=args.chunk, c2w=pose, frame_time=frame_time,
                                                        **render_kwargs_test)
                ...
                img_path = os.path.join(args.outputdir, 'r_{:02d}.png'.format(idx + i_target[-1]))
                predict_image.save(img_path)
